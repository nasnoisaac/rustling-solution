...
fn isValid(r: i16, g: i16, b: i16) -> bool {
    let range = 0..=255;
...
impl TryFrom<(i16, i16, i16)> for Color {
    ...
    fn try_from(tuple: (i16, i16, i16)) -> Result<Self, Self::Error> {
        if isValid(tuple.0, tuple.1, tuple.2) {
            Ok(Color {red: tuple.0 as u8, green: tuple.1 as u8, blue: tuple.2 as u8})
        } else {
            Err(String::from("error from tuple"))
        }
    }
}
...
impl TryFrom<[i16; 3]> for Color {
    ...
    fn try_from(arr: [i16; 3]) -> Result<Self, Self::Error> {
        if isValid(arr[0], arr[1], arr[2]) {
            Ok(Color {red: arr[0] as u8, green: arr[1] as u8, blue: arr[2] as u8})
        } else {
            Err(String::from("error from array"))
        }
    }
}
...
impl TryFrom<&[i16]> for Color {
    ...
    fn try_from(slice: &[i16]) -> Result<Self, Self::Error> {
        if slice.len() == 3 && isValid(slice[0], slice[1], slice[2]) {
            Ok(Color {red: slice[0] as u8, green: slice[1] as u8, blue: slice[2] as u8})
        } else {
            Err(String::from("error from slice"))
        }
    }
}
